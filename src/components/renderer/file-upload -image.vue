...
<template>
  <div>
    ...
    > 
      <!-- <input  type="file" ref="myInput" @change="myUpload" /> -->
    ...
    <div id="cropper" v-if="showmodal" :options="options">
      <div>
        <!-- Modal -->
        <div class="modal fade bd-example-modal-lg" 
          :style="showmodal=== true ? 'display: block; padding-right: 17px;':'display:none;'"
          :class="{show : showmodal}"
          :aria-modal='{true: showmodal=== true}'
          id="staticBackdrop" data-backdrop="static"
          data-keyboard="false" tabindex="-1" 
          :role="{dialog : showmodal===true}"
          aria-labelledby="staticBackdropLabel" 
          :aria-hidden="{true:showmodal===false}"
        >
          <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <button type="button" class="close" data-dismiss="modal" @click="changeModal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <keep-alive>
                  <crop-image :showmodal="showmodal" :localValue="localValue" :options="options" 
                    @updateImageData="addImageData" @toggleModal='changeModal'
                  />
                </keep-alive>
              </div>
            </div>
...
export default {
  ...
  watch: {
    localValue: {
      handler() {
        ...
        console.log(this);
        this.addImageData(this.localValue);
      },
    },
    name: {
      handler() {
  ...
  methods: {
    ...
    addFile(file) {
      ...
      this.options.target = URL.createObjectURL(item);
      if (this.disabled) {
        file.ignored = true;
        return false;
      }
    ...
    start() {
      ...
      //User Edit
      if (this.filesAccept==this.accept){
        this.changeModal();
      }
      else if (this.filesAccept==null){
        this.changeModal();
      }
      else if (!this.filesAccept==this.accept){
        alert('This type file not accepted !');
      }
      //End
